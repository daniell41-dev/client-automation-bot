...
- Fila de 4 stats (Negocios activos, Clientes, Rubros, Leads hoy). **Los stats se calculan de los datos**, no son fijos.
- **Tabla** (grid) columnas: NEGOCIO (tile + nombre), RUBRO, CLIENTE, ESTADO (pill Activo/Pausado con punto), LEADS (Sora), PLAN (pill Pro/Free), y **columna de acciones (⋯)**. Muestra negocios de **todos** los clientes.
- **Menú de fila (⋯):** Ver detalle · Editar · Pausar/Activar bot · Eliminar (rojo). Clic en el nombre del negocio abre el detalle. Clic fuera cierra el menú.

### 12. Back office — Usuarios y clientes
- **Tabla** columnas: USUARIO (avatar + nombre), EMAIL, ROL (pill Cliente/Administrador), NEGOCIOS (cantidad), RUBROS ASIGNADOS, y **acciones (⋯)**. Acción primaria "Invitar usuario".
- **Menú de fila (⋯):** Editar usuario · Eliminar.
...
- **Menú (⋯) en cada card:** Editar plantilla · Eliminar. El link "Editar plantilla" del pie abre el mismo modal.
...
- Sin acción primaria en la topbar (el export CSV fue descartado por ahora).

### 16. Modales de creación / edición (back office)
Mismo shell para los tres: overlay `rgba(16,24,40,.55)`, card máx 520px, `max-height:88vh`, header (título + subtítulo + cerrar), cuerpo scrolleable, footer `#F9FAFB` con Cancelar + acción primaria. Animación `pop .18s`.
- **Nuevo / Editar negocio:** Nombre · **Cliente dueño** (lista con radio) · **Rubro = plantilla base** (lista con tile e indicación de sus campos) · bloque azul **"Campos que hereda este negocio"** (se actualiza al cambiar de rubro) · WhatsApp · Plan (segmented Free/Pro). Al crear, el negocio nace **Pausado** con 0 leads.
- **Invitar / Editar usuario:** Nombre · Email · Rol (segmented Cliente/Administrador) · **Rubros que puede gestionar** (chips multi-selección; sólo si el rol es Cliente — un Administrador ve "Todos").
- **Nueva / Editar plantilla:** Nombre del rubro · **Campos del negocio (su CRUD)**: chips con borrar + input "Agregar campo" + botón Agregar · Tipo de citas · Estado (segmented Borrador/Publicado).
- En modo edición el título pasa a "Editar…" y el botón a "Guardar cambios", con los campos precargados.

### 17. Back office — Detalle de negocio
- Link "← Volver a Negocios".
- **Cabecera:** tile grande, nombre, pills de estado y plan, "{rubro} · {cliente}", botones **Pausar/Activar bot** (outline) y **Editar** (primario).
- **4 métricas** (Leads totales, Resueltas por el bot, Citas del mes, Tiempo de respuesta); si el bot está pausado muestran "—"/0.
- **Card "Plantilla heredada":** chips de campos + "Citas: {tipo}".
- **Card "Datos del negocio":** Cliente dueño, Rubro, WhatsApp (o "Sin conectar"), Leads totales, y acción **Eliminar negocio** en rojo.
...
14. `14-modal-nuevo-negocio.png` — Modal: nuevo negocio (cliente + plantilla + campos heredados)
15. `15-modal-invitar-usuario.png` — Modal: invitar usuario (rol + rubros)
16. `16-modal-nueva-plantilla.png` — Modal: nueva plantilla (campos del CRUD)
17. `17-menu-acciones-fila.png` — Menú de acciones por fila (ver / editar / pausar / eliminar)
18. `18-detalle-negocio.png` — Detalle de negocio (métricas + plantilla heredada + datos)
